#!/usr/bin/env python3
"""
cve_lookup_v2.py — Quarterly vulnerability lookup against the NVD API v2.0

v2 changes from v1:
  - Filenames now carry a version number so old downloads don't get confused
    with new ones.
  - --days no longer defaults to 100; with no date flags it now searches
    all-time, matching the NVD website's default keyword search behavior.
  - New --end-days-ago flag to shift the search window back in time, useful
    for generating a synthetic "old" snapshot to test cve_diff_v2.py against.
  - New --vendor (+ optional --cpe-part) flag: pairs free-text keyword
    search with a vendor-only CPE prefix filter (virtualMatchString), so you
    get keyword search's recall with much less noise, for products where a
    full precise CPE string isn't practical to pin down.
...
python cve_lookup_v2.py --keywords "Femap" "Simcenter" "SolidWorks" --out results.csv

# Same, but restricted to the last 100 days (for quarterly incremental runs)
python cve_lookup_v2.py --keywords "Femap" "Simcenter" "SolidWorks" --days 100 --out results.csv

# Keyword search narrowed to a specific vendor's products (much less noise)
python cve_lookup_v2.py --keywords "femap" --vendor siemens --out femap.csv

# Use a config file listing products (one per line, "#" for comments)
python cve_lookup_v2.py --keywords-file products.txt --days 90 --out results.csv

# Precise search using CPE strings instead of free-text keywords
python cve_lookup_v2.py --cpe "cpe:2.3:a:siemens:femap" --out results.json --format json

# Simulate an "old" snapshot from 100 days ago, to test cve_diff_v2.py
python cve_lookup_v2.py --keywords "ansys" --days 100 --end-days-ago 100 --out ansys_old.csv

# Use an NVD API key (recommended — raises rate limit from 5 to 50 req/30s)
python cve_lookup_v2.py --keywords "Femap" --api-key YOUR_KEY_HERE

Notes
-----
- Free-text keyword search can be noisy (NVD matches against the whole CVE
  description, not just a product-name field). --vendor narrows this by
  ANDing in a vendor-only CPE prefix filter — much easier to get right than
  a full product CPE, since NVD does not document any restriction against
  combining keywordSearch with virtualMatchString this way. Still spot-check
  results, since this interaction isn't explicitly confirmed either way in
  NVD's docs.
...
def build_query_params(keyword=None, cpe_name=None, virtual_match_string=None,
                        start_date=None, end_date=None, start_index=0):
    params = {
...
def fetch_cves_for_term(term, is_cpe, days, api_key, verbose=False, end_days_ago=0,
                         vendor_match_string=None):
    """Fetch all CVE pages for a single keyword or CPE string.

    vendor_match_string, if given, is a virtualMatchString (e.g.
    'cpe:2.3:a:siemens') ANDed together with the keyword search to narrow
    results to one vendor's products. Ignored when is_cpe is True, since a
    full CPE string already pins down the vendor.
    """
    results = []
    start_index = 0
    total_results = None
    ...
    while total_results is None or start_index < total_results:
        params = build_query_params(
            ...
            virtual_match_string=None if is_cpe else vendor_match_string,
...
def main():
    ...
    parser.add_argument("--vendor", default=None,
                        help="Narrow --keywords/--keywords-file search to one vendor by ANDing in a "
                             "vendor-only CPE prefix filter (virtualMatchString), e.g. --vendor siemens. "
                             "Use the vendor name as it appears in NVD's CPE dictionary (lowercase, "
                             "underscores for spaces) — look it up at https://nvd.nist.gov/products/cpe/search "
                             "if unsure. Ignored when using --cpe, since a full CPE already pins the vendor.")
    parser.add_argument("--cpe-part", default="a", choices=["a", "o", "h"],
                        help="CPE 'part' to use when building the --vendor filter: a=application (default), "
                             "o=operating system, h=hardware.")
    ...
    if args.vendor and args.cpe:
        print("Note: --vendor is ignored when using --cpe (a full CPE string already specifies the vendor).",
              file=sys.stderr)

    vendor_match_string = None
    if args.vendor:
        vendor_slug = args.vendor.strip().lower().replace(" ", "_")
        vendor_match_string = f"cpe:2.3:{args.cpe_part}:{vendor_slug}"
    ...
    for term, is_cpe in terms:
        label = "CPE" if is_cpe else "keyword"
        if vendor_match_string and not is_cpe:
            print(f"Searching {label}: {term}  (vendor filter: {vendor_match_string})", file=sys.stderr)
        else:
            print(f"Searching {label}: {term}", file=sys.stderr)
        rows = fetch_cves_for_term(term, is_cpe, args.days, args.api_key, args.verbose,
                                    end_days_ago=args.end_days_ago,
                                    vendor_match_string=vendor_match_string)
