#!/usr/bin/env python3
"""
cve_diff_v2.py — Compare two cve_lookup_v2.py output files and report what's new.

v2: filename now carries a version number to match cve_lookup_v2.py; no
...
Usage
-----
python cve_diff_v2.py --old q2_results.csv --new q3_results.csv --out q3_new.csv

# Also flag entries whose CVSS score or last-modified date changed since last time
python cve_diff_v2.py --old q2_results.csv --new q3_results.csv --out q3_changes.csv --include-modified

# JSON in, JSON out
python cve_diff_v2.py --old q2_results.json --new q3_results.json --out q3_new.json

Input files are expected to have the columns/fields produced by
cve_lookup_v2.py (cve_id, published, last_modified, cvss_score,
cvss_severity, cwe, description, cvss_vector, references, nvd_link,
search_term). Matching is done on cve_id.
"""
